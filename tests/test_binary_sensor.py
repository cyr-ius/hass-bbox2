...
async def test_binary_sensor_link_status(
    hass: HomeAssistant, config_entry: ConfigEntry, router
):
    """Test the link status binary sensor."""
    # --- Test Setup ---
    # Make a deep copy of the fixture data to allow modification in this test
    info_data = copy.deepcopy(INFO)

    # --- Test for ON state ---
    # Set the status to 1 (Connected)
    info_data[0]["device"]["status"] = 1
    router.return_value.device.async_get_bbox_info.return_value = info_data
    ...
    # Check the state
    state = hass.states.get("binary_sensor.bbox_link_status")
    assert state is not None
    assert state.state == STATE_ON

    # --- Test for OFF state ---
    # Set the status to 0 (Disconnected)
    info_data[0]["device"]["status"] = 0
    router.return_value.device.async_get_bbox_info.return_value = info_data
